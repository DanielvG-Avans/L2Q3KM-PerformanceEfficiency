#!/usr/bin/env python3
"""
parse-perfetto.py — Extract energy metrics from Perfetto traces (Samsung A53)

Energy method: battery current × voltage integration (no hardware power rails
on SM-A536B). This is standard practice in mobile energy research — the same
method is used in published papers from MobiSys, IMC, and WWW conferences.

Formula:
  P(t) = |I(t)| × V(t)           # Power in µW (µA × µV / 1e6... wait)
  P(t) = |I_ua(t)| × V_uv(t)     # both in micro-units
  P_mW(t) = P(t) / 1e9           # convert µA·µV → mW  (µA × µV = pW, × 1e-9 = mW)
  E_mJ = ∫ P_mW(t) dt            # integrate over seconds → mJ

Polling interval: 250ms → ~360 samples per 90-second trace.
"""
...
try:
    ...
    PERFETTO_LIB = False
    print("[parse] WARNING: 'perfetto' library not found.")
...
def main():
    ...
    if not json_files:
        print(f"[parse] No run_*.json files in {results_dir}")
        sys.exit(1)

    print(f"[parse] Processing {len(json_files)} runs...")
    rows = []
    ...
    csv_path = results_dir / "energy_metrics.csv"
    df.to_csv(csv_path, index=False)

    print(f"\n[parse] Energy CSV: {csv_path}")
