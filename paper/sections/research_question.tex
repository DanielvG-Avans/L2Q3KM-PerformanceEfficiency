...
\begin{enumerate}
	...
	\item How do selected browser-side metrics, including LCP, FCP, TTFB, navigation response size, and JavaScript heap sampling, differ between SSR and CSR under the same conditions?
	\item To what extent do these browser-side metrics help explain any observed differences in mobile energy use?
...
The working expectation of this study is that SSR will tend to reduce client-side energy use when more of the data-processing workload is completed before delivery to the browser, particularly in the higher-volume scenarios. At the same time, CSR may still perform favorably on some browser-facing metrics. The expectation is therefore not that one strategy will be universally superior, but that the results will show a trade-off between latency-related outcomes and mobile client-side efficiency.
