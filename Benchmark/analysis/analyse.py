#!/usr/bin/env python3
"""
analyse.py — Statistical analysis for SSR vs CSR energy benchmark
Produces: statistics.csv, plots/, report.md
"""
...
def load_data(results_dir: Path) -> pd.DataFrame:
    rows = []
    for jf in sorted(results_dir.glob("run_*.json")):
        with open(jf) as f:
            d = json.load(f)
...
def main():
    ...
    df = load_data(rd)
    if df.empty:
        print("[analyse] No data."); sys.exit(1)

    ssr = df[df["condition"] == "ssr"]
    csr = df[df["condition"] == "csr"]
    print(f"[analyse] {len(ssr)} SSR runs, {len(csr)} CSR runs loaded")
    ...
    print(f"\n[analyse] ✓ report.md, statistics.csv, plots/ saved in {rd}")
